...
function($scope, $rootScope, $log, $tm1Ui,$timeout) {
    ...
    */
    ...
    $rootScope.selections.product = "All Products by Category";
    ...
    $scope.refreshMainData = function(){
        ...
        "rowDimension":{"name":"Product", "mdx":"{TM1DRILLDOWNMEMBER( {[Product].["+$rootScope.selections.product+"]}, ALL, RECURSIVE )}","attributes":"Description"},
        ...
        "cube":"Retail",
        "defaultCubeArray":['Actual',$rootScope.selections.year,'Year','Local',$rootScope.selections.region,$rootScope.selections.product,'Sales Amount'],
    ...
    $scope.loadChartValues = function(one,two,three){
      ...
      ////console.log(one,two,three)
        ...
        $scope.initSelectionProduct = function(){
            ...
            var arrayToKeep = [];
            for(var ttr = 0; ttr < $scope.lists.product.length; ttr++){
                arrayToKeep.push($scope.lists.product[ttr]['key']);
            }
            if( (arrayToKeep).indexOf($rootScope.selections.product) === -1  ){
             //   console.log("NO SELECTED ACCOUNT IN SUBSET - INITIALISE FIRST IN THE LIST");
                if($scope.lists.product.length){
                           $rootScope.selections.product = $scope.lists.product['0']['key'];
                }
