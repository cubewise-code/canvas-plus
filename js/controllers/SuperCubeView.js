(function(){
        ...
        scope.options = {
            "chart": {
              ...
              "legend": {
                dispatch: {
                  legendClick: function(e) {console.log(e, "legendClick")},
                  legendDblclick: function(e) {console.log(e, "legendDblclick")},
                                       ...
                                       //scope.options.chart.margin.left  = (250)*(scope.table.data()[0].elements.length)
                                        ...
                                        for(gggh = 0; gggh < scope.table.data().length; gggh++){
                                            ...
                                            if(scope.randomColor[gggh]){
                                              ...
                                              scope.randomColor[gggh] =  '#' + (0x1000000 + Math.random() * 0xFFFFFF).toString(16).substr(1,6);
                                        ...
                                        for(row in scope.table.data()){
                                            ...
                                            if(scope.consolidatedRowsOnly === true){
                                             ...
                                             for(el in scope.table.data()[row].elements){
                                              ...
                                              if(scope.rowTotalConsolidationArray[row] ){
                                   ...
                                   if(!result.failed){
                                      ...
                                      //console.log(scope.tableDimensionLength ,"scope.tableDimensionLength ");
                                        ...
                                        for(row in scope.table.data()){
                                         ...
                                         if(scope.consolidatedRowsOnly ){
                                          console.log(scope.consolidatedRowsOnly, "CONSOLEDIATED")
                                          if(scope.table.data()[row].elements[0].element['type'] === 'C' || scope.table.data()[row].elements[1].element['type'] === 'C'){
                                            

                                       
                                            scope.charRowCount++;
                                            console.log("CONSOLIDATED ", row,  scope.charRowCount)
                                            var cellArrayFromJson = [];
                                            jsonRowData[row] =  {"key": '',
                                            "color": scope.randomColor[row], "values":[]};
                                            for(var gs = 0; gs < scope.table.data()[row].cells.length; gs++){
