...
directive('pdfviewer', [ '$parse', '$rootScope' , '$timeout', function($parse, $rootScope, $timeout) {
	var canvas = null;
	var instance_id = null;

	return {
		restrict: "E",
		template: '<canvas id="canvas-view"></canvas>',
		scope: {
			onPageLoad: '&',
			loadProgress: '&',
			src: '@',
					...
					$scope.renderPage($scope.pageNum, function(success) {
						...
						if ($scope.loadProgress) {
							...
							$scope.loadProgress({state: "finished", loaded: 0, total: 0});
						  	$rootScope.pdfPageLoaded($scope.pdfDoc);
...
service("PDFViewerService", [ '$rootScope', function($rootScope) {
	...
	svc.gotoPage = function() {
		$rootScope.$broadcast('pdfviewer.gotoPage');
	};
	...
	svc.Instance = function(id) {
		...
		return {
			...
			gotoPage: function(page) {
				console.log("test go to page", page);
